...
fn launch_netgear(ip_address: String, username: String, password: String, ping_ip: Option<std::net::IpAddr>) {
...
fn main() {
    let m = App::new(crate_name!())
        ...
        .arg(Arg::with_name("ip_address")
                ...
                .multiple(false)
             .required(true))
        .arg(Arg::with_name("modem_ip_address")
             .help("The IP address of the modem (usually 192.168.100.1)")
             .takes_value(true)
             .short("i")
             .long("ip")
    ...
    match model {
        Model::CM1000 => launch_netgear(ip_address, username, password, ping_ip),
        Model::CM1100 => println!("Found CM1100"),
        Model::SB8200 => launch_arris(ip_address, ping_ip)
    }
}
